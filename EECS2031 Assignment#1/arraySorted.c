...
int myAdd( int A[], int size, int d ) {
  ...
  int i; // Declaration of variable (index)
  
  if(size < MAX_SIZE) { // If size of array is less then the max size, array will add
    
    for (i = size - 1; i >= 0 && A[i] > d; i--) { // For loop from last element to first element
    // Two conditions to check the position to insert:
    //Index is greater or equal to 0 in order to go through size of array
    // Number at index is bigger then number that will get added 
      A[i+1] = A[i]; // Elements are moved to the right from current cell block to the next cell block
    }

    A[i+1] = d; // Number is added to array
